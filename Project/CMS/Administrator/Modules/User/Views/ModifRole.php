<?php echo $top_menu; ?>
<h1><?php echo UPDATE_ROLE ?></h1>
<?php echo $error; ?>
<form action="index.php?page=User&action=role_edit_update" method="post">
    <div class="row">
        <div class="col-md-12">
        	<table class="table-info table-striped">
                <tr>
                    <td width="20%"><?php echo $form['roles']['role']['label']; ?></td>
                    <td><input type='text' name='user[role]' value='<?php echo $al_fetch_roles->role ?>' required pattern="([A-Za-z, :,0-9]){3,}" />
                </tr>
